...
def dkimid(_msg, _body, _lid, _attachments, raw_msg):
    r"""
    DKIM-ID generator: SHAKE-128 of DKIM relaxed/simple canonicalisation
...
def medium(msg, body, lid, _attachments, _raw_msg):
    ...
    mdatestring = time.strftime(
        "%Y/%m/%d %H:%M:%S", time.gmtime(email.utils.mktime_tz(mdate))
    )
...
def medium_original(msg, body, lid, _attachments, _raw_msg):
    ...
    try:
        ...
        uid_mdate = email.utils.mktime_tz(
            mdate
        )  # Only set if Date header is valid
    except:
        pass
...
# Recommended until September 2020 for cluster setups.
# See 'dkimid' for recommended generation.
...
def cluster(msg, body, lid, attachments, _raw_msg):
    ...
    mdate = None
    mdatestring = (
        "(null)"  # Default to null, ONLY changed if replicable across imports
    )
    try:
        mdate = email.utils.parsedate_tz(msg.get("date"))
        mdatestring = time.strftime(
...
def legacy(msg, body, lid, _attachments, _raw_msg):
    ...
    mid = "%s@%s@%s" % (
        hashlib.sha224(
            body if type(body) is bytes else body.encode("ascii", "ignore")
        ).hexdigest(),
        uid_mdate,
        lid,
    )
